	...
	for (rx=-1; rx<2; rx++)
		for (ry=-1; ry<2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				else if ((sim->elements[TYP(r)].Properties & TYPE_SOLID) && TYP(r) != PT_DMND && TYP(r) != PT_GLAS && TYP(r) != PT_JRAY && parts[i].tmp == 0 && RNG::Ref().chance(int(2 - sim->pv[y / CELL][x / CELL]), 6667))
				{
					sim->part_change_type(i,x,y,PT_CO2);
					parts[i].ctype = 5;
					sim->pv[y/CELL][x/CELL] += 0.2f;
